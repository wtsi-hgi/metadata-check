...
import mcheck.irods.icommands_wrapper as irods_api
...
class TestiRODSMetaQueryOperations(unittest.TestCase):
    ...
    def test_query_by_metadata(self):
        # Testing a normal AVU that 3 files have:
        avu_dict = {"study" :"ARandomStudy"}
        ...
        # Testing what happens when a non-existing AVU is sent:
        avu_dict = {}
        expected = []
        result = irods_api.iRODSMetaQueryOperations.query_by_metadata(avu_dict)
        self.assertSetEqual(set(expected), set(result))
