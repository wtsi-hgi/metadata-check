...
class URLOpener(object):
    ...
    def get_request(self, url):
        ...
        if isinstance(url, string_types):
            url = urllib.request.Request(url, headers={'Accept-encoding': 'identity'})
        return url
    ...
    def get_response(self, url, username=None, password=None):
        ...
        if stored_username is None:
            if username is None and self.prompting:
                username = urllib.parse.quote(input('User for %s: ' % netloc))
                password = urllib.parse.quote(getpass.getpass('Password: '))
            if username and password:
                self.passman.add_password(None, netloc, username, password)
            stored_username, stored_password = self.passman.find_user_password(None, netloc)
        authhandler = urllib.request.HTTPBasicAuthHandler(self.passman)
    ...
    def get_opener(self, *args, **kwargs):
        ...
        if self.proxy_handler:
            args.extend([self.proxy_handler, urllib.request.CacheFTPHandler])

        if kwargs.get('scheme') == 'https':
            https_handler = VerifiedHTTPSHandler()
            director = urllib.request.build_opener(https_handler, *args)
            #strip out HTTPHandler to prevent MITM spoof
            for handler in director.handlers:
                if isinstance(handler, urllib.request.HTTPHandler):
                    director.handlers.remove(handler)
        else:
            director = urllib.request.build_opener(*args)
    ...
    def setup(self, proxystr='', prompting=True):
        ...
        if proxy:
            self.proxy_handler = urllib.request.ProxyHandler({"http": proxy, "ftp": proxy, "https": proxy})
    ...
    def extract_credentials(self, url):
        ...
        if username is None:
            ...
            password = urllib.parse.quote(getpass.getpass(prompt))
        else:
            # remove the auth credentials from the url part
...
class Urllib2HeadRequest(urllib.request.Request):
    def get_method(self):
        return "HEAD"
